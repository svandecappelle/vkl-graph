...
abstract class Chart<T> extends SimplePanel implements HasAttachedChartEventHandlers {
	...
	// Stylish
	this.resizableContainer.addStyleName("gwt-ResizableChart");
	...
	if (this.isResizableChart) {
	    ...
	    this.resizer.bind(this.chartJavascriptObject);
	}
    }
    ...
    var chartJavascript = chartJavascript = $wnd.jQuery.jqplot(elementId, dataString, options);
	...
	if (visible && isNotInjected && attached && isWidthVisible) {
		...
		@Override
		public void execute() {
		    chartJavascriptObject = callJqPlot(getId(), dataController.getInjectionData(), dataController.getInjectionOptions(), isPluginEnable, theme);
		    JsConsole.info("chart successfully created and added to DOM.");
		    injected = true;

		    // fire an event to prevent children than the chart was injected.
		    fireEvent(new AttachedChartEvent());
		}
	    });
	}
	// The chart is repositioned to his previous position.
	this.resizableContainer.setWidget(this.chartContainer);
