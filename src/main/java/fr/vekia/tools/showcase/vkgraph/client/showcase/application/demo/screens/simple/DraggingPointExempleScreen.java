	...
	public DraggingPointExempleScreen() {
		...
		chart.bind(new EditedDataEvent() {
			...
			public void onEvent(EventObject datasOnEvent) {
				GWT.log("edited value: [" + datasOnEvent.getSeriesIndex() + "-"
						+ datasOnEvent.getPointIndex() + "] - {"
						+ datasOnEvent.getValue() + "}");
			}
		});
		...
		gridAttribute.addValueChangeHandler(new ValueChangeHandler<String>() {
			...
			public void onValueChange(ValueChangeEvent<String> event) {
				...
				DraggingPointExempleScreen.this.chart.setTextOption(
						ChartOption.series, SubOption.color,
						gridAttribute.getText());
				DraggingPointExempleScreen.this.chart.replot();
			}
		});

		panel.add(new HTML(
				"<h2>Try to right clic on the chart. And Change the theme on the available selection.</h2>"));
		panel.add(chart);
