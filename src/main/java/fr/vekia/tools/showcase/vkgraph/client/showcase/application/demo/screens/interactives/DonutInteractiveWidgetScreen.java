...
public class DonutInteractiveWidgetScreen extends HorizontalPanel {
	...
	public DonutInteractiveWidgetScreen() {
		...
		pieChart.setNumberOption(ChartOption.seriesDefaults,
		...
		// pieChart.setBooleanOption(ChartOption.seriesDefaults,
		// SubOption.rendererOptions, SubOption.highlightMouseOver, false);
		...
		pieChart.bind(new DataClickEvent() {
			...
			public void onEvent(EventObject datasOnEvent) {
				...
				datasTable.setText(values.get(1).size() + 2, 1,
		...
		datasTable.getRowFormatter().setStyleName(regions.size() + 1,
				"gwt-DataTableTitle");
		datasTable.getRowFormatter().setStyleName(0, "gwt-DataTableTitle");
		datasTable.getRowFormatter().setStyleName(regions.size() + 2,
				"gwt-ImportantLabel");
	...
	private void applyStyleToGrid(final FlexTable table, final int pointIdx) {
		table.getRowFormatter().getElement(pointIdx).getStyle()
	...
	private void applyStyleToGrid(final FlexTable table,
			final Object eventObjectFiredValue) {
		...
		if (eventObjectFiredValue.toString().contains("North")) {
			...
			startIdx = NORTH_REGION_COUNT + CENTER_REGION_COUNT
					+ SOUTH_REGION_COUNT + EAST_REGION_COUNT;
		}
