...
public class ThemingChartScreen extends SimplePanel {
	...
	public ThemingChartScreen() {
		...
		first.setNumberOption(ChartOption.axes, SubOption.yaxis, SubOption.min,
				90);
		first.setBooleanOption(ChartOption.seriesDefaults,
		...
		second.setBooleanOption(ChartOption.seriesDefaults,
				SubOption.rendererOptions, SubOption.showDataLabels, true);
		second.setBooleanOption(ChartOption.seriesDefaults,
				SubOption.showMarker, true);
		...
		third.setBooleanOption(ChartOption.seriesDefaults,
				SubOption.rendererOptions, SubOption.showDataLabels, true);
		third.setBooleanOption(ChartOption.seriesDefaults,
				SubOption.showMarker, true);
	...
	private IsWidget getFlexTableThemeCreator() {
				...
				.addValueChangeHandler(new ValueChangeHandler<String>() {
					...
					public void onValueChange(ValueChangeEvent<String> event) {
						backgroundGridColorBox
								...
								.getStyle()
								.setBackgroundColor(
										backgroundGridColorBox.getText());
					}
				});
		...
		for (int i = 0; i < 10; i++) {
			...
			seriesColor.addValueChangeHandler(new ValueChangeHandler<String>() {
				...
				@Override
				public void onValueChange(ValueChangeEvent<String> event) {
					seriesColor.getElement().getStyle()
							.setBackgroundColor(seriesColor.getText());
				}
			});
			row++;
		}
		...
		generate.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				color.put("backgroundColor", new JSONString(
						backgroundGridColorBox.getText()));
				grid.put("grid", color);
				...
				for (TextBox widget : colors) {
					if (!widget.getText().isEmpty()) {
						...
						colorSerie.put("color",
								new JSONString(widget.getText()));
				...
				first.activateTheme(themeNameBox.getText(),
						grid.getJavaScriptObject());
			}
		});
		row++;

		CheckBox checkBox = new CheckBox(
				"Enable change default theme by right click");
