...
public class Dialog extends Animation {
	...
	protected void onUpdate(double progress) {
		...
		this.messageBox.getElement().getStyle()
				.setLeft(positionX, Style.Unit.PX);
		this.messageBox.getElement().getStyle()
				.setTop(positionY, Style.Unit.PX);
	}
	...
	protected void onComplete() {
		super.onComplete();
		this.messageBox.getElement().getStyle()
				.setLeft(this.finalX, Style.Unit.PX);
		this.messageBox.getElement().getStyle()
				.setTop(this.finalY, Style.Unit.PX);
	}
	...
	public void scrollCenter(final int milliseconds) {
		final Number positionFinalX = Window.getClientWidth()
				* (DEFAULT_WIDGET_ANIMATION_X_END_POSITION / 100f)
				- (messageBox.getOffsetHeight() / 2);
		final Number positionFinalY = Window.getClientHeight()
				* (DEFAULT_WIDGET_ANIMATION_Y_END_POSITION / 100f)
				- (messageBox.getOffsetWidth() / 2);
		scrollTo(positionFinalX.intValue(), positionFinalY.intValue(),
				milliseconds / 2);
	}
	...
	private void show(AnimationEnum animation) {
		...
		if (animation == AnimationEnum.SCROLLCENTER_BOUNCE) {
			...
			timers.addToStack(new SequentialTimer(400) {
				...
				@Override
				public void execute() {
					animate(AnimationEnum.BOUNCE, 3,
							MILLESECONDS_FAST_EVENT_DURATION);
				}
			});

			timers.run();
		} else {
			JqueryAnimation.animateShow(messageBox.getElement(), animation,
					MILLESECONDS_SLOW_EVENT_DURATION);
		}
	}
	 ...
	 */
	public void setResetPositionPopupToEachVisible(
			boolean resetPositionPopupToEachVisible) {
		this.resetPositionPopupToEachVisible = resetPositionPopupToEachVisible;
	}

	public void setPopupAnimationStartPosition(int startxPosition, Unit unitx,
			int startyPosition, Unit unity) {
		this.unitx = unitx;
		this.unity = unity;
	...
	private void resetStartPosition() {
		...
		Number starty;
		if (unitx == Unit.PCT) {
			startx = Window.getClientWidth() * (positionPercentLeft / 100f)
					- (messageBox.getOffsetHeight() / 2);
		} else {
			startx = positionPercentLeft;
		}
		if (unity == Unit.PCT) {
			starty = Window.getClientHeight() * (positionPercentTop / 100f)
					- (messageBox.getOffsetWidth() / 2);
		} else {
	...
	private void hide(AnimationEnum animation) {
		JqueryAnimation.animateHide(messageBox.getElement(), animation,
				MILLESECONDS_SLOW_EVENT_DURATION);
		SequentialTimerStack timers = new SequentialTimerStack();
		timers.addToStack(new SequentialTimer(MILLESECONDS_SLOW_EVENT_DURATION
				+ MILLESECONDS_FAST_EVENT_DURATION) {
	 ...
	 * @param bounce
	 */
	public void animate(AnimationEnum animation, int timesToAnimation,
			int duration) {
		JqueryAnimation.animate(animation, messageBox.getElement(),
				timesToAnimation, duration);
	}
