...
public class BuildYoursExempleScreen extends Composite {
	...
	public BuildYoursExempleScreen() {
		...
		addButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				optionsTable.setWidget(newRowId, 0, new ListChartOption(
						newRowId));
			}
		});
		...
		optionRendererList.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				chartGeneric.setRenderer(RenderersEnum
						.valueOf(optionRendererList.getValue(optionRendererList
								.getSelectedIndex())));
			}
		});
	...
	private class ListChartOption extends ListBox implements HasIsOption {
		...
		public ListChartOption(final int rowId) {
			...
			option = ChartOption.valueOf(ListChartOption.this
					.getValue(ListChartOption.this.getSelectedIndex()));
			changeWidget();
		...
		private void changeWidget() {
			BuildYoursExempleScreen.this.changeWidget(rowId, 1, option, option,
					null, null, this);
		}
	...
	private void apply(ChartOption option, List<SubOption> subOptions,
			Boolean value) {
		if (subOptions != null) {
			...
			case 1:
				...
				break;
			case 2:
				chartGeneric.setBooleanOption(option, subOptions.get(1),
						subOptions.get(0), value);
				break;
			case 3:
				chartGeneric.setOption(option, subOptions.get(2), "{"
						+ subOptions.get(1) + ": {" + subOptions.get(0) + " : "
						+ value + " }");
				break;
			case 4:
				chartGeneric.setBooleanOption(option, subOptions.get(0),
						subOptions.get(1), value);
				break;
			default:
				break;
			}
	...
	private void apply(ChartOption option, List<SubOption> subOptions,
		...
		if (subOptions != null) {
			...
			case 1:
				...
				break;
			case 2:
				chartGeneric.setOption(option, subOptions.get(1),
						subOptions.get(0), value);
				break;
			case 3:
				chartGeneric.setOption(option, subOptions.get(2), "{"
				...
				break;
			case 4:
				chartGeneric.setOption(option, subOptions.get(0),
						subOptions.get(1), value);
				break;
			default:
				break;
	...
	private void apply(ChartOption option, List<SubOption> subOptions,
		...
		if (subOptions != null) {
			...
			case 1:
				...
				break;
			case 2:
				chartGeneric.setNumberOption(option, subOptions.get(1),
						subOptions.get(0), value);
				break;
			case 3:
				chartGeneric.setOption(option, subOptions.get(2), "{"
				...
				break;
			case 4:
				chartGeneric.setNumberOption(option, subOptions.get(0),
						subOptions.get(1), value);
				break;
			default:
				break;
	...
	private class ListSubOption extends ListBox implements HasIsOption {
		 ...
		 * 
		 */
		public ListSubOption(final int rowId, final int nbParentOption,
				IsOption chartOption, HasIsOption parentList) {
			this.rowId = rowId;
			...
			if (ListSubOption.this.getItemCount() > 0) {
				SubOption subOption = SubOption.valueOf(ListSubOption.this
						.getValue(ListSubOption.this.getSelectedIndex()));
			...
			addChangeHandler(new ChangeHandler() {
				...
				@Override
				public void onChange(ChangeEvent event) {
					if (ListSubOption.this.getValue(ListSubOption.this
							.getSelectedIndex()) != "NONE") {
						SubOption subOption = SubOption
								.valueOf(ListSubOption.this
										.getValue(ListSubOption.this
												.getSelectedIndex()));
						changeWidgets(subOption);
		...
		private void changeWidgets(SubOption subOption) {
			...
			BuildYoursExempleScreen.this.changeWidget(rowId,
					nbParentOption + 1, subOption, getOptionChart(), subOption,
					getAllOptions(), this);
		}
	...
	private void changeWidget(final int rowId, final int columnId,
			final HasType type, final ChartOption optionChart,
			SubOption subOption, final List<SubOption> allSubOptions,
			HasIsOption hasIsOption) {
		Widget widget = null;
		if (subOption != null && type.getType() == null) {
			...
			if (type.hasChildren()) {
				ListSubOption listOptions = new ListSubOption(rowId, 1,
						hasIsOption.getOption(), hasIsOption);
			...
			box.addValueChangeHandler(new ValueChangeHandler<String>() {
				...
				public void onValueChange(ValueChangeEvent<String> event) {
					apply(optionChart, allSubOptions,
							Float.parseFloat(event.getValue()));
				}
			});
			widget = box;
		...
		if (widget != null) {
			...
			if (!(widget instanceof ListSubOption)
					&& optionsTable.getCellCount(rowId) > columnId + 1) {
				optionsTable.removeCells(rowId, columnId + 1,
						optionsTable.getCellCount(rowId) - (columnId + 1));
			}
		}
	}
