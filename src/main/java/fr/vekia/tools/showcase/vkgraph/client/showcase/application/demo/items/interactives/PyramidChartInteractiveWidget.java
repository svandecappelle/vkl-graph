/*
 * File: $URL: svn+ssh://chimay/home/svn/VkGraph-showcase/VkGraph-showcase/src/main/java/fr.vekia.tools.showcase.vkgraph/client/showcase/application/components/menu/groups/interactive/items/PyramidChartInteractiveWidget.java $
 * $Id: PyramidChartInteractiveWidget.java 53 2012-09-26 15:38:21Z svandecappelle $
 * Licence MIT
 *
 * Last change:
 ...
 *          {@inheritDoc}
 */
public class PyramidChartInteractiveWidget extends HorizontalPanel {

	/**
	 * Default constructor
	 * 
	 */
	public PyramidChartInteractiveWidget() {

		final PyramidChart chart = getChart();
		final FlexTable datasTable = new FlexTable();
		datasTable.setText(0, 0, "Sex");
		datasTable.setText(1, 0, "Age");
		datasTable.setText(2, 0, "Ratio");

		chart.bind(new DataHighlightEvent() {

			@Override
			public void onEvent(EventObject datasOnEvent) {
				if (datasOnEvent.getSeriesIndex() == 0) {
					datasTable.setText(0, 1, "Male");
				} else {
					datasTable.setText(0, 1, "Female");
				}

				datasTable.setText(1, 1,
						Integer.toString(datasOnEvent.getPointIndex() + 1));
				String ratio = datasOnEvent.getValue().toString()
						.replaceFirst("([0-9])*,", "");
				datasTable.setText(2, 1, ratio);
			}
		});
	...
	private PyramidChart getChart() {
		...
		chart.setNumberOption(ChartOption.seriesDefaults,
				SubOption.rendererOptions, SubOption.barPadding, 2);
		chart.setBooleanOption(ChartOption.seriesDefaults,
				SubOption.rendererOptions, SubOption.offsetBars, true);
		chart.setOption(ChartOption.seriesDefaults, SubOption.yaxis, "yaxis");
		chart.setBooleanOption(ChartOption.seriesDefaults, SubOption.shadow,
				false);
		chart.setOption(ChartOption.axes, SubOption.xaxis,
				SubOption.rendererOptions, "{baselineWidth: 2}");
		chart.setOption(ChartOption.axes, SubOption.yaxis, SubOption.label,
				"Age");
		chart.setOption(ChartOption.axes, SubOption.yaxis,
				SubOption.labelRenderer,
				RenderersEnum.CanvasAxisLabel.getValueRenderer());
		chart.setNumberOption(ChartOption.axes, SubOption.yaxis,
				SubOption.tickInterval, 5);
		chart.setBooleanOption(ChartOption.axes, SubOption.yaxis,
				SubOption.showMinorTicks, true);
		chart.setOption(ChartOption.axes, SubOption.yaxis,
				SubOption.rendererOptions, "{category:false}");
		chart.setOption(ChartOption.axes, SubOption.yaxis, SubOption.ticks,
				getTicks());
		chart.setOption(ChartOption.axes, SubOption.y2axis, SubOption.ticks,
				getTicks());
		chart.setOption(ChartOption.axes, SubOption.y2axis, SubOption.label,
				"Age");
		chart.setNumberOption(ChartOption.axes, SubOption.y2axis,
				SubOption.tickInterval, 5);
		chart.setBooleanOption(ChartOption.axes, SubOption.y2axis,
				SubOption.showMinorTicks, true);
		chart.setOption(ChartOption.axes, SubOption.y2axis,
				SubOption.rendererOptions, "{category:false}");
		...
		maleSeries.setNumberOption(SubOption.rendererOptions,
				SubOption.synchronizeHighlight, 1);

		femaleSeries.setOption(SubOption.rendererOptions, SubOption.side,
				"right");
		femaleSeries.setNumberOption(SubOption.rendererOptions,
				SubOption.synchronizeHighlight, 0);
		femaleSeries.setOption(SubOption.yaxis, "y2axis");

		male2Series
				.setOption(SubOption.rendererOptions, SubOption.side, "left");
		male2Series.setBooleanOption(SubOption.rendererOptions, SubOption.fill,
				false);

		female2Series.setOption(SubOption.rendererOptions, SubOption.side,
				"right");
		female2Series.setBooleanOption(SubOption.rendererOptions,
				SubOption.fill, false);
