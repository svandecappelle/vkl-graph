 ...
 * @version 2.1
 * 
 *          {@inheritDoc} A utility class that loads style sheets.
 */
public class StyleSheetLoader {
	...
	public StyleTesterTimer(String refStyleName, Command callback) {
	    ...
	    RootLayoutPanel.get().add(refWidget);
	}
	...
	public void run() {
		...
		RootLayoutPanel.get().remove(refWidget);
     ...
     * is affected by a rule in the style sheet, as described in {@link #loadStyleSheet(String, String, Command)}. When the style sheet has
