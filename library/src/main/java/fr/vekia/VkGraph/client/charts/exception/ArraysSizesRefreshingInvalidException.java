 ...
 * @version 2.1
 * 
 *          {@inheritDoc} Exception throwed when the refresh graph is required but the data are not valid on sizes. Typically when the graph need to
 *          be replot after data series changes and the series number to refresh is higher than the initial graph. This use case must done with a
 *          complete reinstanciation graph.
 */
